...
class Swin_Cross_Att(nn.Module):
    r""" ConvNeXt
    ...
    """
    def __init__(self, in_chans=3, model_name = 'swin_large', num_classes=1000, mla_channels=256, mla_index = (0,1,2,3), dims=[192, 384, 768,1536], layer_scale_init_value=1e-6, head_init_scale=1., norm_cfg = None, category_emb_dim = 128,scale = 128, pretrain_img_size=224,
                 patch_size=4,
                 embed_dim=192,
                 depths=[2, 2, 18, 2],
    ...
    def init_weights(self, pretrained=None):
        ...
        if self.random_init == False:
            ...
            if self.model_name in ['swin_large']:
                load_pretrained_swin(self)
            else:
                raise ValueError('pretrain data not loaded')
        else:
...
file_path = '/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/work_dirs/swin_best_batch8_iter4/iter_8000.pth'
...
for img in img_list:
  ...
  for i in range(4):
    ...
    for j in range(len(result)):
      ...
      ax = sns.heatmap(result[j], cmap='Greys_r', fmt=".3f")
      plt.title('img')
      os.makedirs(f'/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/swin_cross_att_/', exist_ok = True)
      plt.savefig(f'/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/swin_cross_att_/{img}_{dict_[i]}_{j}.png')
      plt.show()
      plt.close()

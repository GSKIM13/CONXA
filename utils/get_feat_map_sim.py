...
class ConvNeXt_V2_CASE2_LN(nn.Module):
    ...
    def forward(self, x):
        ...
        outs_mla.append(outs[0])
        outs_mla.append((outs[0]+outs[1]))
        outs_mla.append((outs[0]+outs[1]+outs[2]))
        outs_mla.append((outs[0]+outs[1]+outs[2]+outs[3]))
        outs_mla.append((outs[0]+outs[1]+outs[2]+outs[3]))
        outs_mla.append((outs[1]+outs[2]+outs[3]))
        outs_mla.append((outs[2]+outs[3]))
        outs_mla.append(outs[3])
        ...
        return b6_depth, b12_depth, b18_depth, b24_depth, p6_depth, p12_depth, p18_depth, p24_depth
...
for img in img_list:
  ...
  for i in range(8):
      for j in range(8):
          similarity_matrix[i, j] = cosine_similarity(output[i].flatten(), output[j].flatten())

  plt.figure(figsize=(8, 6))
  sns.heatmap(similarity_matrix.numpy(), annot=True, cmap='gray_r', cbar=True)
  plt.title("Cosine Similarity Between 8 Tensors")
  plt.xlabel("Tensor Index")
  plt.ylabel("Tensor Index")
    

  os.makedirs(f'/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/mla_cosine/', exist_ok = True)
  plt.savefig(f'/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/mla_cosine/{img}.png')
  plt.show()
  plt.close()

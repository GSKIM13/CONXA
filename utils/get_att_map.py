...
class ConvNeXt_V2_CASE2_LN(nn.Module):
    ...
    def __init__(self, in_chans=3, model_name = 'convnext_large_224', num_classes=1000, mla_channels=256, mla_index = (0,1,2,3),
                 depths=[3, 3, 27, 3], dims=[192, 384, 768,1536], drop_path_rate=0., drop_rate=0., embed_dim = 256,
    ...
    def forward(self, x):
        ...
        fea_piece = self.category_emb_dim // 4
        cat_piece =  self.attention_dim // 4
        
        '''
        
        for cat in range(4):
          for fea in range(4):
            result = torch.bmm(attention[:,cat*cat_piece:(cat+1)*cat_piece,fea*fea_piece:(fea+1)*fea_piece] * value[:,fea*fea_piece:(fea+1)*fea_piece,height*width])
...
file_path = '/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/work_dirs/ConvNeXt_256_126_64_32_iter2/iter_3000.pth'
...
for img in img_list:
  ...
  for i in range(4):
      for j in range(4):
          ...
          resampled_data[i, j] = block.sum()/32
  
  
  plt.figure(figsize=(8, 6))
  ax = sns.heatmap(resampled_data, cmap='Greys_r', annot = True, fmt=".3f", vmin=0.2, vmax=0.3)
  plt.title('Heatmap of Summed Data (4x4)')
  os.makedirs(f'/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/display/display_{file_name}_{iter_name}', exist_ok = True)
  plt.savefig(f'/home/gwangsoo13kim/EdgeSementic/EDTER_TriDecTr_GS/display/display_{file_name}_{iter_name}/{img}')
  plt.show()
  plt.close()

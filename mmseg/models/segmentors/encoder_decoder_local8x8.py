...
class EncoderDecoder_LOCAL8x8(BaseSegmentor):
    ...
    def _fuse_features_forward_train(self, local_features, global_features):
        """Run forward function and calculate loss for fuse head in
                inference."""
        print('_fuse_features_forward_train')
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        preds_depth = img.new_zeros((batch_size, 2048, h_encode, w_encode)) # backbone model's output dim : 2048*20*20 -> 2048*40*40
        preds_normal = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        preds_ref = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        preds_illu = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_seg_logit = self.extract_feat(crop_img) ##torch.Size([2, 8192, 20, 20])
                
                chunks = torch.chunk(crop_seg_logit, chunks =4, dim =1)
                chunk_depth = chunks[0]
                chunk_normal = chunks[1]
                chunk_ref = chunks[2]
                chunk_illu = chunks[3]

                
                preds_depth[:, :, int(y1 / 8):int(y2 / 8), int(x1 / 8):int(x2 / 8)] = chunk_depth ## (2,2048,20,20)
                preds_normal[:, :, int(y1 / 8):int(y2 / 8), int(x1 / 8):int(x2 / 8)] = chunk_normal
                preds_ref[:, :, int(y1 / 8):int(y2 / 8), int(x1 / 8):int(x2 / 8)] = chunk_ref
                preds_illu[:, :, int(y1 / 8):int(y2 / 8), int(x1 / 8):int(x2 / 8)] = chunk_illu

                count_mat[:, :, int(y1 / 8):int(y2 / 8), int(x1 / 8):int(x2 / 8)] += 1
        assert (count_mat == 0).sum() == 0 # 40*40 scan if any value ==0 then error
        
        x = (preds_depth, preds_normal, preds_ref, preds_illu) ## preds_depth.shape : torch.Size([2, 2048, 40, 40]), 
        
        
        losses = dict()
        loss_decode, local_features = self._decode_head_forward_train(x, img_metas,gt_semantic_seg) #TriDecDecoder's result
        losses.update(loss_decode)

        if self.with_auxiliary_head:
            loss_aux = self._auxiliary_head_forward_train(local_features, img_metas, gt_semantic_seg)
            losses.update(loss_aux)
            

        fuse_loss = self._fuse_head_forward_train(local_features,global_features, gt_semantic_seg) #TriDecDecoder + global_feature
    ...
    def inference_global_local_feature(self, img, img_meta, rescale):
        ...
        preds_depth = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        preds_normal = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        preds_ref = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        preds_illu = img.new_zeros((batch_size, 2048, h_encode, w_encode))
        count_mat = img.new_zeros((batch_size, 1, h_encode, w_encode))
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        """Inference by sliding-window with overlap. input image : 321*481
        ...
        batch_size, _, h_crop_img, w_crop_img = img_crop.size() # (1,3,320,480)
        
        

        global_features_crop = self.global_model.slide_inference_global_features(img_crop, img_meta, rescale).detach() ## input 
        ...
        x_crop = (preds_depth, preds_normal, preds_ref, preds_illu)
        
        local_outs_crop, local_features_crop = self._decode_head_forward_test(x_crop,img_meta)
        
        fuse_outs_crop = self._fuse_head_forward_test(local_features_crop, global_features_crop) # 1,320,480,4
        
        fuse_outs_crop = fuse_outs_crop.permute(0,3,1,2)

...
class EncoderDecoder(BaseSegmentor):
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg):
        ...
        #loss_decode, _ = self.decode_head.forward_train2(x, img_metas,
        #                                             gt_semantic_seg,
        #                                             self.train_cfg)
    ...
    def slide_inference_global_features(self, img, img_meta, rescale):
        """Inference by sliding-window with overlap. 320*480 size img
        ...
        h_grids = max(h_img - h_crop + h_stride - 1, 0) // h_stride + 1
        w_grids = max(w_img - w_crop + w_stride - 1, 0) // w_stride + 1
        preds = img.new_zeros((batch_size, 256, h_img, w_img))     #for BSDS500 128 -> 256
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                y1 = max(y2 - h_crop, 0)
                x1 = max(x2 - w_crop, 0)
                crop_img = img[:, :, y1:y2, x1:x2] #320*320
                
                
                crop_seg_logit = self.encode_decode_global_features(crop_img, img_meta) # 256*320*320

...
class BIMLAHead(nn.Module):
    def __init__(self, mla_channels=256, mlahead_channels=64, norm_cfg=None):
        ...
        self.head2_1 = nn.Sequential(nn.ConvTranspose2d(mla_channels, mlahead_channels, 4, stride=2, padding=1, bias=False), build_norm_layer(norm_cfg, mlahead_channels)[1], nn.ReLU(inplace=True),
                                     nn.ConvTranspose2d(mlahead_channels, mlahead_channels, 8, stride=4, padding=2, bias=False), build_norm_layer(norm_cfg, mlahead_channels)[1], nn.ReLU(inplace=True))
...
class VIT_BIMLAHead_CASE7_8x8(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, mla_channels=256, mlahead_channels=128, 
        ...
        super(VIT_BIMLAHead_CASE7_8x8, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        # inputs_ = inputs[0].chunk(4, dim=1) # inputs[0] : global feature (Stage 1 Data)
        depth = inputs[0]
        normal = inputs[1]
        ref = inputs[2]
        illu = inputs[3]
        
        depth = torch.chunk(depth, chunks = 8, dim = 1)
        normal = torch.chunk(normal, chunks = 8, dim = 1)
        ref = torch.chunk(ref, chunks = 8, dim = 1)
        illu = torch.chunk(illu, chunks = 8, dim = 1)
        ...
        edge_depth = self.edge_depth(x_depth) # channel 1ea reduction

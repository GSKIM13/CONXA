...
class BIMLAHead_LN_ConvNeXt(nn.Module):
    def __init__(self, in_channel = 32, middle_channels=32, head_channels=64, norm_cfg=None):
        ...
        self.head = nn.Sequential(nn.ConvTranspose2d(self.in_channel, self.middle_channels, 4, stride=2, padding=1, bias=False), build_norm_layer(norm_cfg, 
        [self.middle_channels,40,40])[1] , nn.GELU(),
                                     nn.ConvTranspose2d(self.middle_channels, self.head_channels, 16, stride=8, padding=4, bias=False),  build_norm_layer(norm_cfg, 
        [self.head_channels,320,320])[1], nn.GELU())
...
class ViT_Cross_Att_Head(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, middle_channels=256, head_channels=128, 
        ...
        super(ViT_Cross_Att_Head, self).__init__(**kwargs)
        ...
        self.global_features_depth = nn.Sequential(
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), build_norm_layer(self.norm_cfg, [self.head_channels,320,320])[1],
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), nn.GELU(), nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), build_norm_layer(self.norm_cfg, [self.head_channels,320,320])[1], nn.GELU())

...
class VIT_BIMLAHead_CASE3(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, mla_channels=256, mlahead_channels=128, 
        ...
        super(VIT_BIMLAHead_CASE3, self).__init__(**kwargs)
        ...
        self.global_features = nn.Sequential(
            nn.Conv2d(32 * self.mlahead_channels, self.mlahead_channels, 3, padding=1),
            nn.BatchNorm2d(self.mlahead_channels), nn.ReLU(inplace=True),
            nn.Conv2d(self.mlahead_channels, self.mlahead_channels, 3, padding=1),
            nn.BatchNorm2d(self.mlahead_channels), nn.ReLU(inplace=True),
            nn.Conv2d(self.mlahead_channels, self.mlahead_channels, 3, padding=1),
            ...
            nn.Conv2d(self.mlahead_channels, self.mlahead_channels, 1),
            nn.BatchNorm2d(self.mlahead_channels), nn.ReLU(inplace=True))
        self.edge = nn.Conv2d(self.mlahead_channels, 4, 1)


    def forward(self, inputs):
        '''
        x_depth = self.mlahead_depth(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6], inputs[7])
        x_depth = self.global_features_depth(x_depth)
        edge_depth = self.edge_depth(x_depth)
        edge_depth = torch.sigmoid(edge_depth)
        
        x_normal = self.mlahead_normal(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6], inputs[7])
        x_normal = self.global_features_normal(x_normal)
        edge_normal = self.edge_normal(x_normal)
        edge_normal = torch.sigmoid(edge_normal)

        x_ref = self.mlahead_ref(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6], inputs[7])
        x_ref = self.global_features_ref(x_ref)
        edge_ref = self.edge_ref(x_ref)
        edge_ref = torch.sigmoid(edge_ref)

        x_illu = self.mlahead_illu(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6], inputs[7])

...
class VIT_BIMLAHead_CASE4(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, mla_channels=256, mlahead_channels=128, 
        ...
        super(VIT_BIMLAHead_CASE4, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        ...
        x_normal = self.mlahead_normal(inputs[8], inputs[9], inputs[10], inputs[11], inputs[12], inputs[13], inputs[14], inputs[15])
        x_normal = self.global_features_normal(x_normal)
        edge_normal = self.edge_normal(x_normal)
        edge_normal = torch.sigmoid(edge_normal)

        x_ref = self.mlahead_ref(inputs[16], inputs[17], inputs[18], inputs[19], inputs[20], inputs[21], inputs[22], inputs[23])
        x_ref = self.global_features_ref(x_ref)
        edge_ref = self.edge_ref(x_ref)
        edge_ref = torch.sigmoid(edge_ref)

        x_illu = self.mlahead_illu(inputs[24], inputs[25], inputs[26], inputs[27], inputs[28], inputs[29], inputs[30], inputs[31])

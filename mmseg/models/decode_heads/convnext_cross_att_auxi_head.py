...
class ConvNeXt_CROSS_ATT_AUXIHead(BaseDecodeHead):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=768, **kwargs):
        super(ConvNeXt_CROSS_ATT_AUXIHead, self).__init__(**kwargs)
        
        self.img_size = img_size
        #if self.in_channels==8192:
        self.div_channels = int(self.in_channels/4)
        self.div_channels_2 = int(self.in_channels/32)
        #self.div_channels_2 = int(self.in_channels/8)
        #print(self.div_channels)
        self.aux_depth = nn.Sequential(
                nn.ConvTranspose2d(self.div_channels, self.div_channels, 2, stride=2, padding=0, bias=False),
                ###nn.ConvTranspose2d(self.in_channels, 1, 16, stride=8, padding=4, bias=False),
                nn.ConvTranspose2d(self.div_channels, 1, 2, stride=2, padding=0, bias=False)

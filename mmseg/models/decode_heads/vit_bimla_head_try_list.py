...
class VIT_BIMLAHead(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, mla_channels=256, mlahead_channels=128, 
        ...
        for i in range (self.num_class):
            self.mlahead.append(BIMLAHead(mla_channels=self.mla_channels, mlahead_channels=self.mlahead_channels, norm_cfg=self.norm_cfg))
            self.global_features.append(nn.Sequential(
            ...
            nn.BatchNorm2d(self.mlahead_channels), nn.ReLU()))
            self.edge.append(nn.Conv2d(self.mlahead_channels, 1, 1))
        '''
    ...
    def forward(self, inputs):
        ...
        edge = []
        for i in range (self.num_class):
            x.append(self.mlahead[i](inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5], inputs[6], inputs[7]))
            x[i] = self.global_features[i](x[i])
            edge.append(self.edge[i](x))
            edge[i] = torch.sigmoid(edge[i])
        return edge, x

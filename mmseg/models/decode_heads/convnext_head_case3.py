...
class BIMLAHead_LN_ConvNeXt(nn.Module):
    def __init__(self, in_channel = 32, middle_channels=32, head_channels=64, norm_cfg=None):
        ...
        [self.middle_channels,160,160])[1], nn.ConvTranspose2d(self.middle_channels, self.head_channels, 2, stride=2, bias=False), nn.GELU())
...
class ConvNeXt_Head_CASE3(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, middle_channels=256, head_channels=128, 
        ...
        super(ConvNeXt_Head_CASE3, self).__init__(**kwargs)
        ...
        self.global_features_depth = nn.Sequential(
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), build_norm_layer(self.norm_cfg, [self.head_channels,320,320])[1], nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), nn.GELU(), nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), build_norm_layer(self.norm_cfg, [self.head_channels,320,320])[1],nn.Conv2d(self.head_channels, self.head_channels, 1), nn.GELU())

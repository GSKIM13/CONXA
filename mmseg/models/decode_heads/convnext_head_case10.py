...
class BIMLAHead_BN_ConvNeXt(nn.Module):
    def __init__(self, in_channel = 32, middle_channels=32, head_channels=64, norm_cfg=None):
        ...
        self.head = nn.Sequential(nn.ConvTranspose2d(self.in_channel, self.middle_channels, 2, stride=2,  bias=False), build_norm_layer(norm_cfg, 
        128)[1], nn.ReLU(inplace=True), nn.ConvTranspose2d(self.middle_channels, self.head_channels, 2, stride=2, bias=False),  build_norm_layer(norm_cfg, 
        256)[1], nn.ReLU(inplace=True))
...
class ConvNeXt_Head_CASE10(BaseDecodeHead):
    ...
    def __init__(self, img_size=768, middle_channels=256, head_channels=128, 
        ...
        super(ConvNeXt_Head_CASE10, self).__init__(**kwargs)
        ...
        self.norm_cfg_1 = dict(type='SyncBN', requires_grad=True)
        ...
        self.global_features_depth = nn.Sequential(
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1))            
        self.edge_depth = nn.Sequential(nn.Conv2d(self.head_channels, 1, 1),  build_norm_layer(self.norm_cfg, 1)[1])
        ...
        self.global_features_normal = nn.Sequential(
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1))            
        self.edge_normal = nn.Sequential(nn.Conv2d(self.head_channels, 1, 1),  build_norm_layer(self.norm_cfg, 1)[1])
        ...
        self.global_features_ref = nn.Sequential(
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1))            
        self.edge_ref = nn.Sequential(nn.Conv2d(self.head_channels, 1, 1),  build_norm_layer(self.norm_cfg, 1)[1])
        ...
        self.global_features_illu = nn.Sequential(
            nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1), nn.Conv2d(self.head_channels, self.head_channels, 3, padding=1))            
        self.edge_illu = nn.Sequential(nn.Conv2d(self.head_channels, 1, 1),  build_norm_layer(self.norm_cfg, 1)[1])

...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=False, num_patches=576, align_corners=False):
    ...
    if num_classes == 1000 and cfg['num_classes'] == 1001:  #19, 1000
...
def load_pretrained_convnext(model, cfg=None, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=False, num_patches=576, align_corners=False):
    if cfg is None:
        cfg = getattr(model, 'default_cfg')
    if cfg is None or 'url' not in cfg or not cfg['url']:
        _logger.warning("Pretrained model URL is invalid, using random initialization.")
...
def load_pretrained_swin(model, cfg=None, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=False, num_patches=576, align_corners=False):
  
    #model.load_state_dict(state_dict, strict=strict)
    
    checkpoint = torch.load('/home/gwangsoo13kim/EdgeSementic/SWIN-RIND/model_zoo/swin_large_patch4_window7_224_22k.pth')
    
    state_dict = model.state_dict()
  

    pretrained_dict = {f"{k}": v for k, v in checkpoint['model'].items()}
            
    load_dict = {k: v for k, v in pretrained_dict.items() if k in state_dict}
            
    if not load_dict:
      print("Warning: No matching parameters found. Please check the model and checkpoint compatibility.")
    ...
    if load_dict:
        ...
        print('No matching parameters found in the checkpoint.')
    
def load_pretrained_resnet(model, cfg=None, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=False, num_patches=576, align_corners=False):
    if cfg is None:
        cfg = getattr(model, 'default_cfg')
    if cfg is None or 'url' not in cfg or not cfg['url']:
        _logger.warning("Pretrained model URL is invalid, using random initialization.")

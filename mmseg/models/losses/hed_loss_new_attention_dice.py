...
class HEDLoss_NEW_ATTENTION_DICE(nn.Module):
    """HEDLoss.
    Args:
    ...
    def __init__(self,
                 ...
                 dice_coeff = 3000,
                 delta = 0.1):
        super(HEDLoss_NEW_ATTENTION_DICE, self).__init__()
    ...
    def hed_attention_loss(self, pred,
        ...
        for b_i in range(batch):
          for c in range(4):
            ...
            weight = t * (alpha**self.delta) * (self.beta ** ((1.0 - p_clip) ** self.gamma)) + \
             (1.0 - t) * (1.0 - (alpha**self.delta)) * (self.beta ** (p_clip ** self.gamma))
    ...
    def forward(self,
        ...
        dice = self.dice_loss(cls_score, label)
        
        
        loss_cls += self.dice_coeff * dice 

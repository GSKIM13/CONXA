...
class HEDLoss_ATTENTION_TVERSKY(nn.Module):
    """HEDLoss.
    Args:
    ...
    def __init__(self,
                 ...
                 sigma1 = 0.5,
                 sigma2 = 0.5):
        super(HEDLoss_ATTENTION_TVERSKY, self).__init__()
    ...
    def dice_loss_old(self, pred, target, smooth=1):
    ...
    def teversky_loss(self, pred, target, smooth=1):
        pred = pred.contiguous()
        target = target.contiguous()    
    
        intersection = (pred * target).sum(dim=2).sum(dim=2)
        
        false_positive = ((1-target) * pred).sum(dim=2).sum(dim=2)
        false_negative = (target*(1-pred)).sum(dim=2).sum(dim=2)
        
        
        
        tversky_index = (intersection + smooth) / (intersection + self.sigma1 * false_positive + self.sigma2 * false_negative + smooth)
    ...
    def forward(self,
        ...
        tversky = self.teversky_loss(cls_score, label)
        
        
        loss_cls += self.dice_coeff * tversky

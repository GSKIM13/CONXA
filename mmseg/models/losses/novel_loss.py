...
class NovelLoss(nn.Module):
    ...
    def dice_loss(self, pred, target, smooth=1e-6):
        pred = pred.contiguous() # 4,4,320,320
        target = target.contiguous()    
        ...
        for i in range(c):
          pred_ = pred[:,i,:,:]
          target_ = target[:,i,:,:]
          intersection = (pred_ * target_).sum(dim=1).sum(dim=1)        
          loss = (1 - ((2. * intersection + smooth) / ((pred_**2).sum(dim=1).sum(dim=1) + (target_**2).sum(dim=1).sum(dim=1) + smooth)))
          total_loss += loss.sum()       
    ...
    def forward(self,
        ...
        #print(f'attention loss : {loss_cls}, dice_loss : {dice}, rev_dice = {rev_dice}, iou = {iou} ')
